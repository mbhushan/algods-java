/**
 ...
 it moves in that direction. Also, "level" means the height of the terrain plus any water in that column.
 We can assume there's infinitely high terrain on the two sides out of bounds of the array.
 Also, there could not be partial water being spread out evenly on more than 1 grid block - each unit
...
public class PourWater {
    ...
    public int[] pourWater(int[] heights, int V, int K) {
        for (int i=0; i<V; i++) {
            int minIndex = getFirstLeft(heights, K);
            if (minIndex == K) {
                minIndex = getFirstRight(heights, K);
            }
            ++heights[minIndex];
    ...
    private int getFirstLeft(int [] heights, int K) {
        int min  = heights[K];
        int minIndex = K;
        for (int i=K-1; i>=0; --i) {
            if (heights[i] > min) {
    ...
    private int getFirstRight(int [] heights, int K) {
        int min  = heights[K];
        int minIndex = K;
