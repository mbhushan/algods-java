...
/**
 ...
 respectively) in directory root/d1/d2/.../dm. Note that n >= 1 and m >= 0. If m = 0, it
...
public class DuplicateFile {
    ...
    public static void main(String[] args) {
        ...
        String [] in = {"root/a 1.txt(abcd) 2.txt(efgh)", "root/c 3.txt(abcd)", "root/c/d 4.txt(efgh)", "root 4.txt" +
                "(efgh)"};
    ...
    public List<List<String>> findDuplicate(String[] paths) {
        ...
        for (String s: paths) {
            ...
            for (int i=1; i<arr.length; i++) {
                String [] fArr = arr[i].split("\\(");
                String fileName = fArr[0];
                String content = fArr[1].substring(0, fArr[1].length() - 1);
                List<String> fList = fmap.getOrDefault(content, new ArrayList<>());
                fList.add(root + "/" + fileName);
                fmap.put(content, fList);
            }
        }

        for (String key: fmap.keySet()) {
            List<String> list = fmap.get(key);
